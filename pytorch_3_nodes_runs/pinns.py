...
def Laplacian(outputs, xy_inputs):
    ...
    dx, dy = dxy[:,0], dxy[:,1]
    dxy2 = torch.autograd.grad(dx, xy_inputs, torch.ones_like(dx), create_graph=True)[0]# computes d^2u/dx^2
    dx2 =  dxy2[:,0]
    dyx2 = torch.autograd.grad(dy, xy_inputs, torch.ones_like(dy), create_graph=True)[0]# computes d^2u/dy^2
    dy2 =  dyx2[:,1]
    return dx2 + dy2
...
class Net_sep_dense_normalised(Net_base):
    ...
    def forward(self, x):
        ...
        #x = [self.softplus(x) for x in intermediates_xs]
        return x
